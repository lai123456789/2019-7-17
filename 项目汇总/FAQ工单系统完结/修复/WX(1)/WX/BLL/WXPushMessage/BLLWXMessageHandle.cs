    ...
    public class BLLWXMessageHandle
    {
        ...
        public string CustomTextMsgHandle(Dictionary<string, string> xmlModel)
        {
            ...
            if (xmlModel["MsgType"] == "text")
            {
                ...
                {
                    string wxopenid = xmlModel["FromUserName"];
                    content = "你也好！" + wxopenid;
                }
                ...
                {
                    string st = xmlModel["Content"];
                    string id = xmlModel["FromUserName"];
                    string r = System.Text.RegularExpressions.Regex.Replace(st, @"[^0-9]+", "");
                    BLLSendWXTemplateMsg mysend = new BLLSendWXTemplateMsg();
                    string errmsg = "";
                    BLLUserManage myuser = new BLLUserManage();
                    bool re = myuser.Getopenid(id, r, ref errmsg);
                    bool success = mysend.SendSys(WXApiInfo.wxappid, "oeZqt5nMy4bUdx4WBv8TVhSa7Gb8", r, id, DateTime.Now.ToString("yyyy/MM/dd HH:mm:ss"), ref errmsg);
