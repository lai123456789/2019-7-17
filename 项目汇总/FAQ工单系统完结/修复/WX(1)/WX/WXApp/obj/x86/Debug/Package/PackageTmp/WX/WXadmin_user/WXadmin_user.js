        ...
        success: function (data) {
            if (data.errcode == 0) {
                ...
                $.each(data.data, function (idx, obj) {                    
                    ...
                    content += '<p id="create_time" class="weui-cell__ft weui-cells__title">联系人：' + obj.name + '</p>';
                    content += '</div>';
                    content += '</div>';
                    content += '</a>';
                });
...
$(document).on('click', '#bianji', function () {                 //编辑确定模态框弹出开始
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            if (data.errcode == 0) {
                weui.alert("编辑成功！");
                setTimeout(function(){         	
                location.href = "WXadmin_user.html";  //修改之后  还是跳转回这个页面 刷新一遍
                },2000);              
