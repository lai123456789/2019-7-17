        ...
        success: function (data) {
            if (data.errcode == 0) {
                ...
                $.each(data.data1, function (idx, obj) {
                    ...
                    content += '<a href="question_click_info.aspx?Wid=' + obj.WORDERCODE + '">'; //点击链接查看 进入详情
                    content += '<div class="weui-cells">';  //状态
                    content += '<div class="weui-cell">'; //工单编号
                    content += '<div class="weui-cell__bd">'; //创建时间
                    content += '<p id="type" class="weui-cells__title weui-cell__hd" style="font-size: 16px;">部门：' + obj.WORDTYPECODE + '</p>';
                    content += '</div>';
                    content += '<p id="status" class="weui-cell__ft" style="color: red;"><span class="weui-badge weui-badge_dot" id="badge"></span>' + state + '</p>';  //状态
                    content += '</div>';
                    content += '<div class="weui-cell">';
                    content += '<div class="weui-cell__hd"><img src=""></div>';
                    content += '<div class="weui-cell__bd">';
                    content += '<p id="content" class="weui-cells__title weui-cell__hd" style="font-size: 16px;">工单标题：' + obj.APPTITTLE + '</p>';
