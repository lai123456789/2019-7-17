...
function add1() {
    ...
    $.ajax({
        ...
        success: function (data) {
            if (data.errcode == 0) {
                alert("添加新用户成功！");
                window.location.href = "admin_user.html?Wid=" + usercode + '&username=' + username + '&usertype=' + usertype + '&token=' + token;
            }
            else {
                alert(data.errmsg);
            }
    ...
    if (data.data.length > 0) {
        ...
        $.each(data.data, function (idx, obj) {
            ...
            if (obj.account == 'admin') {
                ...
                if (obj.usertype == 1) {
                    content += '        <a class="guanliyuan" href="#" style="margin-left:10px;" data-usertype="2">设置为管理员</a>';
                   // content += '        <a class="cjguanliyuan" href="#" style="margin-left:10px;" data-usertype="3">设置为超级管理员</a>';
                }
                if (obj.usertype == 2) {
                    content += '        <a class="putongyonghu" href="#" style="margin-left:10px;" data-usertype="1">设置为普通用户</a>';
                  //  content += '        <a class="cjguanliyuan" href="#" style="margin-left:10px;" data-usertype="3">设置为超级管理员</a>';
                }
                if (obj.usertype == 3) {
                    content += '        <a class="putongyonghu" href="#" style="margin-left:10px;" data-usertype="1">设置为普通用户</a>';
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            if (data.errcode == 0) {
                ...
                alert("删除成功！！！");
                //location.href = "admin_user.html?Wid=" + usercode + '&username=' + username + '&usertype=' + usertype + '&token=' + token;
                GetUserList(token);
            }
            else {
                alert(data.errmsg)
...
$(document).on('click', '#add_sure2', function () {    //
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            if (data.errcode == 0) {
                alert("修改成功！");
                //location.href = "admin_user.html?Wid=" + usercode + '&username=' + username + '&usertype='+usertype + '&token=' + token;  //修改之后  还是跳转回这个页面 刷新一遍
                GetUserList(token);
            }
            else {
                alert(data.errmsg)
...
$(document).on('click', '.cjguanliyuan', function () {
    var id = $(this).parents('tr').attr('id');
    var usercode = $('#' + id).find('td.usercode').text();
    var usertype = $('#' + id).find('td a.cjguanliyuan').attr("data-usertype");
    GetUpdatetype(usercode, usertype);
});
$(document).on('click', '.guanliyuan', function () {
    var id = $(this).parents('tr').attr('id');
    var usercode = $('#' + id).find('td.usercode').text();
    var usertype = $('#' + id).find('td a.guanliyuan').attr("data-usertype");
    GetUpdatetype(usercode, usertype);
});
$(document).on('click', '.putongyonghu', function () {
    var id = $(this).parents('tr').attr('id');
    var usercode = $('#' + id).find('td.usercode').text();
    var usertype = $('#' + id).find('td a.putongyonghu').attr("data-usertype");
    GetUpdatetype(usercode, usertype);
});
