    ...
    $.fn.excelTable = function (option, everything) {
        ...
        if (typeof option == "string") {
            ...
            option.columns.unshift({
                field: 'btn', title: "", align: 'center', width: 30, type: "RowNumber", formatter: function (value, row, index) {
                    return index + 1;
                }
            });
            options = option;
...
$(document).on("click", "#btn", function () {
    var content = "";
    content += '<tr>'
    content += '<td style="text-align: center;background:#D6D6D6"></td>'
    ...
    content += '<td style="text-align: center;"><input type="date" value=""></td>'
    content += '<td style="text-align: center;">天</td>'
    ...
    $("#selectedParts-excel tbody").append(content);
...
var tableVal = [];
$(function () {
    onload = function () {  //页面一开始打开就展示demo1的内容
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                $.each(data.data, function (idx, obj) {
                    content += '<tr>'
                    content += '<td style="text-align: center;width:20px;">' + idx + '</td>'
                    ...
                    switch (obj.progress) {
                        case "100%":
                            state = '已完成100%';
                            $(".percent").css("background", "green");
                            break;
                        case obj.progress:
                            state = obj.progress;
                            break;                        
                    }                    
                    content += '<td style="text-align: center;" class="percent">' + state + '</td>'
                ...
                //$("#selectedParts-excel tbody tr").each(function () {
                //    var trVal = [];  //获取每一行
                //    $(this).find('td:gt(0)').each(function () {
                //        var tdVal = $(this);//.text();  //获取每一行的所有td
                //        trVal.push(tdVal);//    
                //    });
                //    var RES负责人 = trVal[4].text();
                //    if (RES负责人 == "100%") {
                //        $(trVal[4]).css("background", "green");
                //    }
                //});
...
function tijiao() {
    ...
    var rows = $("#selectedParts-excel tbody tr");
    var 明细 = new Array();
    tableVal = [];
    $("#selectedParts-excel tbody tr").each(function () {
        var trVal = [];  //获取每一行
        ...
        // tableVal.push(trVal); 
        var Item = trVal[0].text();
        var Sprint内容 = trVal[1].text();
        var Activity行动 = trVal[2].text();
        var Progress进度 = trVal[3].text();
        var RES负责人 = trVal[4].text();

        var plantime = trVal[8].find("input").val(); //计划开始日期
        var planendtime = trVal[12].find("input").val();//计划结束日期

        //console.log("8" + trVal[8].find("input").val())


        //console.log("12" + trVal[12].find("input").val())


        var usercode = $("#userinfo").attr("data-usercode");
        var data = {
            "Item": Item, "Sprint内容": Sprint内容, "Activity行动": Activity行动,
            "Progress进度": Progress进度, "RES负责人": RES负责人, "usercode": usercode, "plantime": plantime, "planendtime": planendtime
        };
...
//$(document).on('click', '#selectedParts-excel > tbody > tr > td:eq(4)', function () {
