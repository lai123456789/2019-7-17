...
namespace BLL
{
    public class BLLRequestData
    {
        ...
        public bool RequestIISData(string pageurl, string usercode, string data, ref string json, ref string errmsg)
        {
            bool result = true;
            try
            {
                ...
                string url = pageurl;
                json = PalmAPIHelper.PalmCoreIISAPI(url, postdata, "POST");
            }
            catch (Exception ex)
            {
        ...
        /// <param name="errmsg">出错时返回的错误提示信息</param>
        /// <returns>是否成功执行</returns>
        public bool RequestIIS3380Data(string pageurl, string data, string usercode, ref string json, ref string errmsg)
        {
            bool result = true;
            try
            {
