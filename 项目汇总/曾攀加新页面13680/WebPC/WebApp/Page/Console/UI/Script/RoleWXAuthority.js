...
$(function () {
    ...
    全局搜索 = search;
    全局页码 = 1;
    GetList("IIS3380", "AuthorityManage.aspx?action=getrolelist", {
        "page": 1,
        "search": search
    });
})
...
$(document).on("keydown", document, function (e) {
    ...
    if (ev.keyCode == 13) {
        if ($('#search_content').is(':focus')) {
            ...
            全局搜索 = search;
            全局页码 = 1;
            GetList("IIS3380", "AuthorityManage.aspx?action=getallpcpage", {
                "search": search,
                "page": 1
            });
        }
...
$(document).on('click', '#add_sure', function () {
    ...
    AddInfo("IIS3380", "AuthorityManage.aspx?action=addroleinfo", {
        "rolename": rolename,
        "remark": remark,
        "usercode": ""
    });
...
$(document).on('click', '#set_sure', function () {
    ...
    SetInfo("IIS3380", "AuthorityManage.aspx?action=setroleinfo", {
        "rolecode": rolecode,
        "rolename": rolename,
        "remark": remark
    });
...
$(document).on('click', '#datalist2 > .panel-heading > .sava.contor', function () {
    ...
    SavaRoleInfo("IIS3380", "AuthorityManage.aspx?action=roleauthority", {
        "rolecode": rolecode,
        "rolepage": rolepage,
        "usercode": ""
    });
...
$(document).on('click', '#form_tb > tr', function () {
    ...
    var rolecode = $('#form_tb > tr.contor > .td_rolecode').text();
    GetRolePageList("IIS3380", "AuthorityManage.aspx?action=getrolepagelist", {
        "rolecode": rolecode
    });
...
function DelUserInfo(urltype, pageurl, data) {
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            if (data.errcode == 0) {
                alert('删除成功');
                GetList("IIS3380", "PageManage.aspx?action=getallpcpage", {
                    "search": 全局搜索,
                    "page": 全局页码
                });
            }
