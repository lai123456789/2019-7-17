...
$(function () {
    ...
    全局搜索 = search;
    全局页码 = 1;
    GetList("IIS3676", "ProductionPlan/MyProductionPlan.aspx?action=getMachineProductiondef", {});
...
$(document).on('click', '#set_sure', function () {
    ...
    SetInfo("IIS", "/NotifySetting/BusinessNotifySetting.aspx?action=setnotifytypelist", {
        "bcode": bcode,
        "name": name,
        "remark": remark,
        "usercode":""
...
$(document).on('click', '#form_tb2 td a.del', function () {
    ...
    if (confirm("确定要删除用户“" + username + "”的权限吗？")) {
        DelUserBind("IIS3738", "/DeviceManage/DeviceManage.ashx?action=DeleteUserBind", {
            "usercode": usercode,
            "medfcode": medfcode
        });
    }
...
$(document).on('click', '#form_tb > tr', function () {
    ...
    //    "rolecode": rolecode
    //});
    medfcode = $(this).attr('data-val');

    if (medfcode != null && medfcode != '') {
        GetBindUserList("IIS3738", "/DeviceManage/DeviceManage.ashx?action=GetUserBindList", { "medfcode": medfcode });
    }
        ...
        if ($(this).hasClass('contor')) {
            ...
            if (isshow) {
                ...
                if (superiorcode == pagecode) {
                    $(obj).parents('ol.dProjectManaged-list').prev('li.dd-item.dd3-item').find('i.choice').removeClass('contor');
                }
            }
...
$(document).on('click', '#set_bindusersure', function () {
    ...
    if (用户代码 != null && 用户代码 != '') {
        BindUserList("IIS3738", "/DeviceManage/DeviceManage.ashx?action=BindUserList", {
            "medfcode": medfcode,
            "usercode": 用户代码,
            "operation_usercode":""
        });
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            if (data.errcode == 0) {
                alert('绑定成功');
                if (medfcode != null && medfcode != '') {
                    GetBindUserList("IIS3738", "/DeviceManage/DeviceManage.ashx?action=GetUserBindList", {
                        "medfcode": medfcode
                    });
                }
            }
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            if (data.errcode == 0) {
                ...
                var content = '';
                $.each(data.data, function (idx, obj) {
                    content += '<tr  data-val="'+obj.medfcode+'">';
...
删除用户列表
*/
function DelUserBind(urltype, pageurl, data) {
    showLoading();
    $.ajax({
        cache: true,
        type: "POST",
