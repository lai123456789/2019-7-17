...
$(document).on('click', '#form_tb2 td a.del', function () {
    ...
    if (confirm("确定要删除用户“" + username + "”的信息吗？")) {
        DelUserInfo("IIS", "/NotifySetting/BusinessNotifySetting.aspx?action=deluserbind", {
            "usercode": usercode,
            "bcode": bcode
        });
    }
...
$(document).on('click', '#set_bindusersure', function () {
    ...
    var bcode = $('#form_tb > tr.contor > td.td_bcode').text().trim();
    if (用户代码 != null && 用户代码 != '') {
        BindUserList("IIS", "/NotifySetting/BusinessNotifySetting.aspx?action=notifybinduser", {
            "bcode": bcode,
            "usercode": 用户代码
        });
    }
