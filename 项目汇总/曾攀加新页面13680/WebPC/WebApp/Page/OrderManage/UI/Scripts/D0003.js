...
$(function () {
    ...
    全局搜索 = search;
    全局页码 = 1;
    //GetList("IIS", "/NotifySetting/BusinessNotifySetting.aspx?action=getnotifytypelist", {});


    GetUserList("IIS", "/NotifySetting/BusinessNotifySetting.aspx?action=getuserlist", {});

    GetAuditList("IIS3380", "/AdvanceOrder.aspx?action=selectAudit", {});
...
$(document).on('click', '#datalist > .panel-heading > .add-audit', function () {
...
//        return;
//    }
//    AddInfo("IIS", "/NotifySetting/BusinessNotifySetting.aspx?action=addnotifytypelist", {
//        "name": name,
//        "remark": remark,
//        "usercode": ""
//    });
...
$(document).on('click', '#add_sure', function () {
    ...
    AddAudit("IIS3380", "/AdvanceOrder.aspx?action=addAudit", {
        "name": name
    });
...
$(document).on('click', '#form_tb > tr > td > a.edit', function () {
    ...
    var remark = $(this).attr("data-sort");

    $('#set_bcode').text(bcode);
    $('#set_bname').val(bname);
    $('#set_audit').val(remark);
...
$(document).on('click', '#set_sure', function () {
    ...
    var remark = $('#set_audit').val();
    EditAudit("IIS3380", "/AdvanceOrder.aspx?action=editAudit", {
        "code": bcode,
        "name": name,
        "sort": remark
    });
...
$(document).on('click', '#form_tb > tr > td > a.del', function () {
    ...
    if (confirm("确定删除这行数据吗？")) {
        DeleteAudit("IIS3380", "/AdvanceOrder.aspx?action=deleteAudit", {
            "usercode": "",
            "code": bcode
        });
    }
...
$(document).on('click', '#form_tb2 td a.del', function () {
    var bcode = $('#form_tb > tr.contor').attr('data-code').trim();
    var usercode = $(this).attr('data-usercode');
    var username = $(this).attr('data-username');
    if (confirm("确定要删除用户“" + username + "”的信息吗？")) {
        DelUserInfo("IIS3380", "/AdvanceOrder.aspx?action=deleteAuditUser", {
            "usercode": usercode,
            "code": bcode
        });
    }
...
$(document).on('click', '#form_tb > tr', function () {
    ...
    var bcode = $('#form_tb > tr.contor').attr('data-code').trim();
    if (bcode != null && bcode != '') {
        GetBindUserList("IIS3380", "/AdvanceOrder.aspx?action=selectAuditUser", {
            "usercode": "",
            "code": bcode
        });
    }
...
$(document).on('click', '#set_bindusersure', function () {
    ...
    var bcode = $('#form_tb > tr.contor').attr('data-code').trim();
    if (用户代码 != null && 用户代码 != '') {
        BindUserList("IIS3380", "/AdvanceOrder.aspx?action=AddAuditUser", {
            "code": bcode,
            "usercode": 用户代码
        });
    }
...
//                if (bcode != null && bcode != '') {
//                    GetBindUserList("IIS", "/NotifySetting/BusinessNotifySetting.aspx?action=getuserbindlist", {
//                        "bcode": bcode
//                    });
//                }
//            }
...
/*
    添加未下单审核级别定义
*/
function AddAudit(urltype, pageurl, data) {
    showLoading();
    $.ajax({
        cache: true,
        type: "POST",
...
/*
    更新未下单审核级别定义
*/
function EditAudit(urltype, pageurl, data) {
    showLoading();
    $.ajax({
        cache: true,
        type: "POST",
...
/*
    删除未下单审核级别定义
*/
function DeleteAudit(urltype, pageurl, data) {
    showLoading();
    $.ajax({
        cache: true,
        type: "POST",
...
/*
    查看未下单审核级别定义
*/
function GetAuditList(urltype, pageurl, data) {
    showLoading();
    $.ajax({
        cache: true,
        type: "POST",
        ...
        success: function (data) {
            ...
            if (data.errcode == 0) {
                if (data.data.length > 0) {
                    var content = '';
                    $.each(data.data, function (idx, obj) {
                        content += '<tr data-code="'+ obj.code +  '">';
                        content += '<td>'+ obj.sort +'</td>';
                        content += '<td>' + obj.name + '</td>';
                        content += '<td>';
                        content += '        <a href="JavaScript:;" class="edit" data-bcode="' + obj.code + '" data-sort="' + obj.sort +'" data-name="' + obj.name + '">编辑</a>';
                        content += '        <a href="JavaScript:;" class="del"  data-bcode="' + obj.code + '">删除</a>';
                        content += '</td>';
                    });
                    $('#form_tb').empty();
                    $('#form_tb').append(content);
