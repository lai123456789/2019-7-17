...
var 全局页码 = 1;
$(function () {
    GetList("IIS", "ReportTable/SlamaterialAreport.aspx?action=getslamaterialareportcg", {
        "page": 1,
        "search": 全局搜索
    });
})
...
$(document).on('click', '#add_sure', function () {
    ...
    AddInfo("IIS", "ReportTable/SlamaterialAreport.aspx?action=addslamaterialareportcg", {
...
$(document).on('click', '#form_tb > tr > td > a.edit', function () {
    ...
    var condition = $(obj_tr).find('.td_condition').attr('data-val');
    var expected = $(obj_tr).find('.td_expected').attr('data-val');
    var slaimpact = $(obj_tr).find('.td_slaimpact').attr('data-val');
    var erroritem = $(obj_tr).find('.td_erroritem').attr('data-val');
    var coordinateitem = $(obj_tr).find('.td_coordinateitem').attr('data-val');
    var todayemphasis = $(obj_tr).find('.td_todayemphasis').attr('data-val');
...
$(document).on('click', '#set_sure', function () {
    ...
    SetInfo("IIS", "ReportTable/SlamaterialAreport.aspx?action=setslamaterialareportcg", {
        "reportdate": reportdate,
        "ordernum": ordernum,
        "materialnum": materialnum,
        "condition": condition,
        ...
        success: function (data) {
            ...
            if (data.errcode == 0) {
                ...
                $.each(data.data, function (idx, obj) {
                    ...
                    content += '    <td class="td_condition" data-val="' + obj.当前状况 + '">' + obj.当前状况.replace(/\n/g, "<br />") + '</td>';
                    content += '    <td class="td_expected" data-val="' + obj.预计交付日期 + '">' + obj.预计交付日期.replace(/\n/g, "<br />") + '</td>';
                    content += '    <td class="td_slaimpact" data-val="' + obj.sla交付影响 + '">' + obj.sla交付影响.replace(/\n/g, "<br />") + '</td>';
                    content += '    <td class="td_erroritem" data-val="' + obj.异常事项 + '">' + obj.异常事项.replace(/\n/g, "<br />") + '</td>';
                    content += '    <td class="td_coordinateitem" data-val="' + obj.协调事项 + '">' + obj.协调事项.replace(/\n/g, "<br />") + '</td>';
                    content += '    <td class="td_todayemphasis" data-val="' + obj.今日重点事项 + '">' + obj.今日重点事项.replace(/\n/g, "<br />") + '</td>';
