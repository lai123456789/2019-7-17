...
$(document).on('click', '#datalist .download', function () {
    ...
    $('#urltype').val('IIS3380');
    $('#pageurl').val('AdvanceOrder.aspx?action=getExcelLog');
...
$(document).on('click', '#form_th > tr > th', function () {
    ...
    enddate = $('#enddate').val();
    RequestData("IIS3380", "AdvanceOrder.aspx?action=getExcelLog", {
        "onetype": onetype,
        "oneval": oneval,
        "numbertype": numbertype,
        "page": currPage,
        ...
        success: function (data) {
            ...
            if (data.errcode == 0) {
                ...
                $.each(data.data, function (idx, obj) {
                    ...
                    content += '    <td style="cursor:pointer;" title="' + obj.处理状态 + '">' + obj.处理状态 + '</td>';

                    if (obj.结果状态 == "0") {
                        content += '    <td style="color: #43ec0e;">' + '成功' + '</td>';
                    } else if (obj.结果状态 == "1") {
                        content += '    <td style="color: #ea0808;">' + '失败' + '</td>';
                    } else {
                        content += '    <td style="color: #676161;">' + '未处理' + '</td>';
                    }
