	...
	var bigAutocomplete = new function(){
		...
		this.autocomplete = function(param){
			...
			function autocompleteSelect(){
				...
				$bigAutocompleteContent.width(config.width + parseFloat(100));
				...
				if(data != null && $.isArray(data) ){
					...
					for (var i = 0; i < data.length; i++) {
					    if (title.indexOf('|')) {
					        ...
					        if (data[i][splitarr[0]] && data[i][splitarr[0]].toUpperCase().indexOf(keyword_.toUpperCase()) > -1) {
					            data_.push(data[i]);
                                
					        }
					        else if (data[i][splitarr[1]] && data[i][splitarr[1]].toUpperCase().indexOf(keyword_.toUpperCase()) > -1) {
					            data_.push(data[i]);
					        }
