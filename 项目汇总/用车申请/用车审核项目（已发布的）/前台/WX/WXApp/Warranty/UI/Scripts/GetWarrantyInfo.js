...
//    salescode1 = getQueryString("dcode");
...
//    //请求下单图片('IIS3380', '/SalesManage.aspx?action=GetImg', pj);
//    //请求处理图片('IIS3380', '/SalesManage.aspx?action=GetdealImg', pj);
//});
...
//    stopDefault(e);
//    var pj = $('#gratetype_dummy').val();
//    $('#gratetype_dummy').val(pj);
//});
...
//        alert("故障已经解决");
//    } else if ($('#pagethree > .bottom_footer > .left > .box1.contor').text() == "返回") {
//        history.go(-1);
//    }
//});
...
//                    if (是否为状态人 != "1") { $('#pagethree > .bottom_footer > .left > .box1.contor').css('background', '#BD7121'); }
                    

//                    $('#pagethree > .bottom_footer > .right > .box1.contor').text("消息");
//                    $('#pagethree > .bottom_footer > .left > .box1.contor').text("确认编辑");

//                    deal = obj.dealingcode;

//                    var usercode = getQueryString("usercode");

//                    if (是否为状态人 == "1") {

//                        $('#pagethree > .content > .row1 > .box3').css('display', 'none');
//                        $('#pagethree > .content > .row2 > .box3').css('display', 'none');

//                        $('#infotype').remove();
//                        $('#pagethree > .content > .row3 > .box2').text(obj.factory);

//                        $('#gratetype').remove();
//                        $('#pagethree > .content > .row4 > .left > .box2').text(obj.question);
                       
//                        $('#deliverydate').remove();
//                        $('#pagethree > .content > .row8 > .box2').text(obj.breakdowndate);

//                        $('#pagethree > .content > .row1 > .box2 input').attr("readonly", "");
//                        $('#pagethree > .content > .row2 > .box2 input').attr("readonly", "");
//                        $('#gratetype').attr("disabled", "disabled")
//                        $('#infotype').attr("disabled", "disabled")
//                        $('#deliverydate').attr("readonly", "");
//                        //$('#pagethree > .content > .row4 > .right > .box2 input').attr("readonly", "");
//                        $('#pagethree > .content > .row5 > .box2 textarea').attr("readonly", "");
//                        $('#pagethree > .content > .row6 > .box2 textarea').attr("readonly", "");
//                        $('#pagethree > .content > .row7 > .box2 textarea').attr("readonly", "");

//                        $('#pagethree > .bottom_footer > .right > .box1.contor').text("回复");

//                        $('#pagethree > .bottom_footer > .left > .box1.contor').text("返回");
                        
//                    }else if (usercode != $('#usercode').val()) {
//                            $('#pagethree > .content > .row1 > .box3').css('display', 'none');
//                            $('#pagethree > .content > .row2 > .box3').css('display', 'none');
...
//                            if (obj.dealing == "") {
//                                $('#pagethree > .bottom_footer > .left > .box1.contor').text("处理故障");
//                            } else if (obj.consumingdate != "") {
//                                $('#pagethree > .bottom_footer > .left > .box1.contor').text("故障已解决");
//                            } else {
//                                $('#pagethree > .bottom_footer > .left > .box1.contor').text("解决故障");
//                            }
//                    }
...
//                if (salescode1.length != 0) {

//                    获取是否为抄送人('IIS3380', '/SalesManage.aspx?action=GetNofityTypeUser', {
//                        "pj": salescode1,
//                        "usercode": $('#usercode').val()
//                    });
...
//                    请求下单图片('IIS3380', '/SalesManage.aspx?action=GetImg', salescode1);
//                    请求处理图片('IIS3380', '/SalesManage.aspx?action=GetdealImg', salescode1);
...
//                    });
//                }
//                $('#pagethree > .content > .row4 > .right > .box2 input').text(data.num);
...
//   获取是否为抄送人
//*/
//function 获取是否为抄送人(urltype, pageurl, data) {
...
$(document).on('tap', '#pagethree > .bottom_footer > .left > .box1.contor', function (e) {
    ...
    if ($('#pagethree > .bottom_footer > .left > .box1.contor').text() == "处理故障") {
        ...
        if (usercode == deal) {
            解除故障('IIS3380', '/SalesManage.aspx?action=updateSalesRemovedate', salescode1);
        } else {
            alert("您不是处理人,只有处理人才能解除故障");
        }
    } else if ($('#pagethree > .bottom_footer > .left > .box1.contor').text() == "确认编辑") {
        ...
        var data11 = {
            "questionkey": questionkey, "questionvalue": questionvalue, "equipment": equipment, "devicename": devicename, "factory": factory,
            "question": question, "parts": parts, "fault": fault, "treatment": treatment,
            "breakdowndate": breakdowndate, "SpecifyUsers": 推送人员, "dealing": username, "salescode": salescode1, "base64": 图片
        };
    ...
    $.ajax({
        ...
        success: function (data) {
            if (data.errcode == 0)//提取成功
            {
                ...
                //var salescode = $('#salescode').val();
                if (salescode1.length != 0) {
                    根据salescode获取售后数据('IIS3380', '/SalesManage.aspx?action=GetSalesServiceInfo', salescode1);
            ...
            if (data.errcode == 0)//提取成功
            {
                ...
                if (data.data != null) {
                    $.each(data.data, function (idx, obj) {
                        ...
                        deal = obj.dealingcode;
                        usercodesss = obj.removecode;
                        var usercode = usercodesss;
