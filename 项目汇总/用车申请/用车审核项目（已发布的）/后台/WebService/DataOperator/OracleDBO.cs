...
namespace DataOperator
{
    public class OracleDBO
    {
        ...
        private string _ConnStr = System.Configuration.ConfigurationManager.AppSettings["WXAutoDBConnStr"];
        ...
        {
            public OracleCmdParam(string Name, OracleDbType Type, object Value)
            {
                this.Name = Name;
                this.Type = Type;
                this.Value = Value;
        ...
        /// 分页SQL
        /// </summary>
        /// <param name="sql">传入原SQL</param>
        /// <param name="oparam">传入原参数</param>
        /// <param name="page">第几页</param>
        /// <param name="pagesize">每页显示的条数</param>
        /// <param name="errmsg">出错时返回的错误提示信息</param>
        /// <returns>是否成功执行</returns>
        public static bool PagingSQL(ref string sql, ref List<OracleDBO.OracleCmdParam> oparam, int page, int pagesize, ref string errmsg)
        {
            bool result = true;
            try
            {
                string new_sql = @"select *
                                  from (select paging1.*, rownum 序号
                                          from (" + sql + @") paging1
                                         where rownum <= :maxrownum) paging2
                                 where paging2.序号 > :startrownum";
                sql = new_sql;

                oparam.Add(OracleDBO.Param("maxrownum", page * pagesize));
                oparam.Add(OracleDBO.Param("startrownum", pagesize * (page - 1)));
            }
            catch (Exception ex)
            {
        ...
        /// <param name="MyCmdParam">Oracle参数数组.</param>
        /// <returns>返回一个整数.</returns>
        public int ExecuteNonQuery(string ExOracle, List<OracleCmdParam> MyCmdParam)
        {
            int result = 0;
            try
            {
                MyComm.CommandText = ExOracle;
                FillCmdParam(ref MyComm, MyCmdParam.ToArray());
                result = MyComm.ExecuteNonQuery();
            }
            catch (Exception Ex)
            {
        ...
        /// <param name="MyCmdParam">Oracle参数数组.</param>
        /// <returns>返回一个DataReader.</returns>
        public OracleDataReader ExecuteReader(string ExOracle, List<OracleCmdParam> MyCmdParam)
        {
            OracleDataReader MyDr = null;
            try
            {
        ...
        /// <param name="MyCmdParam">Oracle参数数组.</param>
        /// <returns>返回一个object.</returns>
        public object ExecuteScalar(string ExOracle, List<OracleDBO.OracleCmdParam> MyCmdParam)
        {
            object result = null;
            try
            {
        ...
        /// <param name="MyCmdParam">Oracle参数数组.</param>
        /// <returns>返回一个DataSet.</returns>
        public DataSet GetDataSet(string ExOracle, List<OracleCmdParam> MyCmdParam)
        {
            DataSet MyDs = new DataSet();
            try
            {
                GetDataAdapter(ExOracle, MyCmdParam.ToArray()).Fill(MyDs);
            }
            catch (Exception Ex)
            {
                HandelException(Ex);
        ...
        /// <param name="MyCmdParam">Oracle参数数组.</param>
        /// <returns>返回一个DataTable.</returns>
        public DataTable GetDataTable(string ExOracle, List<OracleCmdParam> MyCmdParam)
        {
            DataTable MyTable = new DataTable();
            try
            {
                GetDataAdapter(ExOracle, MyCmdParam.ToArray()).Fill(MyTable);
            }
            catch (Exception Ex)
            {
                HandelException(Ex);
        ...
        /// <param name="MyCmdParam">Oracle参数数组.</param>
        /// <returns>返回一个DataRow.</returns>
        public DataRow GetDataRow(string ExOracle, List<OracleCmdParam> MyCmdParam)
        {
            try
            {
                DataTable dt = GetDataTable(ExOracle, MyCmdParam);
