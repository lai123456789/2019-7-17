...
延时还车按钮
*/
$(document).on('tap', '#operation_btn3 > .submit_btn1', function (e) {
    stopDefault(e);
    $('#undo_moda4').addClass('contor');
    $('#mask300').addClass('contor');


});

$(document).on('tap', '#undo_moda4 > .bottom > .btn_sure', function () {
    var 撤销单号 = getQueryString('pcode');
    //  var 公里数 = $('#remark').text();
    var 原因 = $('#remark4').val();

    //这里调用Ajax方法
    提交原因('IIS3382', '/Proposer.aspx?action=DelayedCar', {
        "applycode": 撤销单号,
        "delayedtime": 原因,
    });
});
...
取消撤回按钮
*/
$(document).on('tap', '#undo_moda4 > .bottom > .btn_cancel', function (e) {
    stopDefault(e);
    $('#undo_moda4').removeClass('contor');
    setTimeout(function () {
...
function 提交原因(urltype, pageurl, data) {
    $.ajax({
        cache: true,
        type: "POST",
        dataType: "json",
            ...
            if (data.errcode == 0) {
                ...
                $.each(data.data, function (idx, obj) {
                    ...
                    if (类型 == 1) { //未审批请假申请
                        ...
                        content += '        <div class="right">5时06分</div>';
                        content += '    </div>';
                        ...
                        content += '        <div class="right status1">已审批</div>';
                        content += '    </div>';
                        content += '    <div class="rows8">';
                    ...
                    if (类型 == 2) {  //同意请假申请
                        ...
                        content += '    <div class="rows8">';
                        content += '        <div class="left">代理审核：</div>';
                        content += '        <div class="right" style="color:green">已同意</div>';
                        content += '    </div>';
                        content += '    <div class="rows8">';
                        content += '        <div class="left">经理审核：</div>';
                        content += '        <div class="right" style="color:green">已同意</div>';
                        ...
                        content += '    <div class="rows8">';
                        content += '        <div class="left">领导审核：</div>';
                        content += '        <div class="right" style="color:green">已批核</div>';
                        content += '    </div>';
                    }
                    if (类型 == 3) {
                        content += '    <div class="rows1">';
                    ...
                    if (类型 == 4) {  //拒绝请假申请
                        ...
                        content += '    <div class="rows8">';
                        content += '        <div class="left">驳回原因：</div>';
                        content += '        <div class="right">节假日前后不得请假</div>';
                        content += '    </div>';
                        //content += '    <div class="rows9">';
                        //content += '        <div class="left">人事审核：</div>';
                        //content += '        <div class="right status1">未审批</div>';
